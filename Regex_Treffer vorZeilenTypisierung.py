...
class RegexTrefferApp:
    ...
    def init_database_schema(self):
        ...
        try:
            ...
            # Falls die Tabelle nicht existiert oder zeile_inhalt_orig fehlt, erstelle neue Tabelle
            if 'zeile_inhalt_orig' not in existing_columns:
        ...
        try:
            ...
            for row_id, zeile_inhalt in rows:
                ...
                if pattern.match(zeile_inhalt):
                    ...
                    cursor.execute('''
                        INSERT INTO Treffer (extracted_data_id, zeile_inhalt, zeile_inhalt_orig, regex_pattern, quell_tabelle, quell_feld, Hit_Platz, Hit_StartNr, Hit_Zeit, gefunden_am)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (row_id, zeile_bereinigt, zeile_inhalt_orig, self.regex_pattern, self.ausgewaehlte_tabelle, self.ausgewaehltes_feld, hit_platz, hit_startnr, hit_zeit, datetime.now()))
                    treffer_count += 1
    ...
    def tabelle_anzeigen(self):
        ...
        try:
            ...
            # Treffer abrufen
            cursor.execute('''
                SELECT T.id, T.extracted_data_id, T.zeile_inhalt, T.zeile_inhalt_orig, T.regex_pattern, T.quell_tabelle, T.quell_feld, T.Hit_Platz, T.Hit_StartNr, T.Hit_Zeit, T.gefunden_am
                FROM Treffer T
                ORDER BY T.id DESC
            ''')
            ...
            # Spalten definieren
            spalten = ("ID", "Quell_ID", "Zeile_Inhalt", "Zeile_Inhalt_Orig", "Regex_Pattern", "Quell_Tabelle", "Quell_Feld", "Hit_Platz", "Hit_StartNr", "Hit_Zeit", "Gefunden_am")
